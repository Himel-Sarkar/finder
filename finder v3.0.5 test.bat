...
echo 18. faganfinder,openknowledgemaps,searchinfo[pdf]
...
cls
echo for bufferover cmd: curl -s dns.bufferover.run/dns?q=.hackthissite.org" 
echo for subdomains enumiration cmd: curl -s "web.archive.org/cdx/search/cdx?url=*.hackthissite.org/*&output=text&fl=original&collapse=urlkey"
echo for subdomains enumiration cmd (2): curl https://api.hackertarget.com/hostsearch/?q=hackthissite.org
echo Finding site endpoints using CommonCrawl: curl -s "http://index.commoncrawl.org/CC-MAIN-2018-22-index?url=*.hackthissite.org&output=json"
start "" https://archive.is/SyGnt
pause
goto vu
)
...
start "" https://cse.google.com/cse?cx=partner-pub-3256770407637090%3A2007347459"&"ie=UTF-8"&"q=%v%"&"sa=Search"&"siteurl=www.irongeek.com%2F"&"ref="&"ss=0j0j1
...
start "" http://www.securitytube.net/?q=%v%
)  
